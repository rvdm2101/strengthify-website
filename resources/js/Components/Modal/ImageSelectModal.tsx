    ...
    <Modal show={show} onClose={onClose} maxWidth="7xl">
        <section className="p-6 grid grid-cols-6 gap-4">
            {images.length ? images.map(image => (
                <button key={image.id} className="bg-white shadow-lg hover:drop-shadow-md active:drop-shadow-lg rounded-2xl border border-gray-500 active:border-indigo-500 overflow-hidden" type="button" onClick={() => selectImage(image.id)}>
